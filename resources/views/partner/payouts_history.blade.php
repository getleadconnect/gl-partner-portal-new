...
.gl-payouts-toolbar .filter-group { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
...
/* Date range filter */
.gl-payouts-toolbar .date-range { display: inline-flex; align-items: center; gap: 6px; margin-left: 4px; }
...
.gl-payouts-toolbar .date_clear:hover { border-color: #DC2626; color: #DC2626; }
...
/* Lead cell — avatar */
.gl-payouts .row-avatar { display: inline-flex; align-items: center; gap: 10px; text-decoration: none; }
    ...
    <div class="container-fluid">
        ...
        <div class="gl-page-header">
            <div class="gl-page-header__text">
                ...
                <div class="gl-page-subtitle">Your commission ledger and payment receipts.</div>
            </div>
            <div class="gl-page-header__actions">
                <a href="{{ route('partner.leads') }}" class="gl-btn gl-btn-outline">
                    <i class="bx bx-arrow-back"></i> My Leads
                </a>
            </div>
        </div>
                                ...
                                <div class="gl-payouts-toolbar" style="border-top:1px solid #F0F2F5;">
                                    <div class="filter-pills" role="tablist">
                                        ...
                                        <div class="date-range">
                                            ...
                                            <input type="date" id="filter_date_from" class="date-range__input">
                                            <label class="date-range__label">To</label>
                                            <input type="date" id="filter_date_to" class="date-range__input">
                            ...
                            <div class="tab-pane fade" id="paid-tab-pane" role="tabpanel" tabindex="0">
                                <div class="gl-payouts-toolbar" style="border-top:1px solid #F0F2F5;">
                                    <div class="filter-pills">
                                        <div class="date-range">
                                            <label class="date-range__label">From</label>
                                            <input type="date" id="filter_date_from_1" class="date-range__input">
                                            <label class="date-range__label">To</label>
                                            <input type="date" id="filter_date_to_1" class="date-range__input">
                                            <button type="button" id="filter_date_clear_1" class="date_clear" title="Clear date range"><i class="bx bx-x"></i></button>
                                        </div>
                                    </div>
                                    <div class="payouts-toolbar__actions">
...
$(function () {
    ...
    // ---- Tab visual sync ----
    document.querySelectorAll('.gl-payouts-toolbar .tabs .tab').forEach(function (btn) {
        btn.addEventListener('click', function () {
            document.querySelectorAll('.gl-payouts-toolbar .tabs .tab').forEach(function (b) {
    ...
    var table = $('#payments_table').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                d.status_filter = $('#payments-tab-pane .filter-pills .filter-pill.active').data('status') || '';
    ...
    // Status pill filter (first tab)
    $('#payments-tab-pane').on('click', '.filter-pills .filter-pill', function () {
        $('#payments-tab-pane .filter-pills .filter-pill').removeClass('active');
        $(this).addClass('active');
        table.ajax.reload();
    });

    // Date range listeners (first tab)
    $('#filter_date_from, #filter_date_to').on('change', function () { table.ajax.reload(); });
...
$("#paid-tab").click(function()
{
    ...
    // Date range (second tab)
    $('#filter_date_from_1, #filter_date_to_1').on('change', function () { table2.ajax.reload(); });
    ...
    $('#export_payout_details').on('click', function () {
        ...
        var lnk = "{{ route('partner.export-payout-details') }}" + "?" + $.param(params);
    ...
    $('#export_payment_history').on('click', function () {
        ...
        var lnk = "{{ route('partner.export-payment-history') }}" + "?" + $.param(params);
