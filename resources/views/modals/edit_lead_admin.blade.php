<form  id="edit-lead-form">
	...
	<input type="hidden" id="edit_country_name" name="edit_country_name" value="{{$lead->country_name}}">
	...
	<div class="row" style="margin:0px 10px ;">
		...
		<div class="col-md-6 form-group">
			<label for="recipient-name" class="form-control-label">Mobile Number <span
					style="color: red;">*</span></label>
			<input type="hidden" class="form-control" placeholder="" value="{{ $lead->country_code }}"  name="edit_country_code" id="edit_country_code" minlength=6 maxlength=15 required>
			<br>
			<input type="tel" class="form-control" placeholder=""
				value="{{'+'.$lead->country_code.$lead->mobile }}"  name="edit_mobile" id="edit_mobile" required>
		</div>
		
		<div class="col-md-6 form-group">
			<label for="recipient-name" class="form-control-label">Company Name(your firm)<span style="color: red;">*</span></label>
			<input type="text" class="form-control" placeholder=""	value="{{ $lead->company_name }}" name="edit_company_name" id="edit_company_name" required>
		</div>
		<div class="col-md-6 form-group">
			<label for="recipient-name" class="form-control-label">Designation</label>
			<input type="text" class="form-control" placeholder=""	value="{{ $lead->designation}}" name="edit_designation" id="edit_designation" >
		</div>
		<div class="col-md-6 form-group">
			<label for="recipient-name" class="form-control-label">Email</label>
			<input type="email" class="form-control" placeholder=""	value="{{ $lead->email}}" name="edit_email" id="edit_email">
		</div>
		<div class="col-md-6 form-group">
...
$('#edit-lead-form').validate({ 
                ...
                submitHandler: function(form) 
                {
					...
					$("#edit_country_code").val(ccode);
								
                    $.ajax({
                        url: "{{ route('admin.update-lead') }}",
                        method: 'post',
                    ...
                    success: function(result)
                    {
                        if(result.states)
                        {
                            ...
                            $.each(result.states, function(key, value) {
                                $('#edit_state').append('<option value="'+ key +'">'+ value +'</option>');
                            });
                        }
                    }
                });
