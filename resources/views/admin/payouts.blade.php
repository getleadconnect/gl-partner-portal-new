...
.cursor-pointer
{
 ...
 <div class="page-content">
	...
	<div class="modal fade" id="view-receipt-modal" tabindex="-1" aria-labelledby="addPartnerModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="addPartnerModalLabel">View Details</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
			<div class="modal-body">
			
			<div class="form-group pd-view">
				<p>Commission : <span id="payCommission"> </span></p>
				<p>Payment Date : <span id="paymentDate"> </span></p>
				<p>Payment Id : <span id="paymentId"> </span></p>
				<p>Receipt :</p>
				<img src="#" id="payReceipt" style="width:100%">
			</div>
...
$("#unpaid_partner_filter").change(function()
 ...
 var table2 = $('#paid_leads_table').DataTable({
			...
			{data: 'partner', name: 'partner'},
			{data: 'status', name: 'status',className:"full-width-select"},
			{data: 'total_collected', name: 'total_collected',className:"numericCol"},
			{data: 'total_commission', name: 'total_commission',className:"numericCol"},
			{data: 'total_paid', name: 'total_paid',className:"numericCol"},
...
/// SET PAYMENT DETAILS -------------same functions added into leads blade file-------------------------------------------------------
 ...
 var paymentValidator=$('#paymentForm').validate({ 
		...
		$("#unpaid_leads_table tbody").on( 'click', '.btn-pay', function ()
		  {
			...
			var Result=$("#set-payment-modal .modal-body");
					...
					jQuery.ajax({
					type: "GET",
					url: "{{url('admin/get-partner-payment-details')}}"+"/"+id,
					dataType: 'html',
					//data: {vid: vid},
  ...
  $(document).on('click','#btnLeadCommSubmit',function()
		...
		$("#unpaid_leads_table tbody").on('click','.close-payment',function()
        {
			
			var lead_id=$(this).attr('data-leadid');
			
			Swal.fire({
			  title: "Are you sure? You want to close this lead payment!",
			  ...
			  if (result.isConfirmed) {
				...
				$.ajax({
					url: "{{ route('admin.update-payout-payment-status') }}",
					type: 'post',
					dataType:'json',
					data: {'_token':"{{csrf_token()}}",lead_id:lead_id,pay_status:1},
					success: function(result)
					{
						if(result.status==1)
						{
