...
<div class="modal fade" id="payment-detail-modal" tabindex="-1" aria-labelledby="addPartnerModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-md">
		<div class="modal-content">
			...
			@csrf
			<input type="hidden" class="form-control" name="pay_lead_id" id="pay_lead_id">
			<input type="hidden" class="form-control" name="lead_pay_status" id="lead_pay_status">
			<input type="hidden" class="form-control" name="pay_partner_id" id="pay_partner_id">
						
			<div class="form-group">
			<div class="row">
			<div class="col-lg-4 col-xl-4 col-xxl-4">
				<label for="recipient-name" class="form-label">Amount</label>
				  <input type="text" class="form-control"  name="pay_amount" id="pay_amount" required readonly>
			</div>
			<div class="col-lg-4 col-xl-4 col-xxl-4">
				<label for="recipient-name" class="form-label">Percentage</label>
				  <input type="text" class="form-control"  name="pay_percentage" id="pay_percentage" required readonly>
			</div>
			<div class="col-lg-4 col-xl-4 col-xxl-4">
				<label for="recipient-name" class="form-label">Commission</label>
				  <input type="text" class="form-control"  name="pay_commission" id="pay_commission" required readonly>
			</div>
...
<script type="text/javascript">
    $(function () {
        ...
        var table = $('#leads-table').DataTable({
            ...
            {data: 'pay_status', name: 'pay_status',className:"full-width-select"},
            {data: 'action', name: 'action'},
            ],
			
            "drawCallback": function( json ) {

                /*if($('#partner_filter').val() != 0)
                {
                    $("#partner_details").remove()
                    $('thead').prepend('<tr id="partner_details"><th colspan="4">Partner Name : '+$('#partner_filter').find(":selected").text()+'</th><th colspan="5">Leads Generated : '+json.aoData.length+'</th></tr>')
                }*/
        ...
        // `d` is the original data object for the row
            ...
            '<tr>'+'<td>Amount:</td>'+'<td class="t-amt">'+d.amount_collected+'</td><td >Commission:</td><td class="t-amt">'+d.commission_amount+'</td></tr>'+
            '</table>'
            );
        }
        
        $('#leads-table tbody').on('click', 'td.dt-control', function () 
        {
...
/// SET PAYMENT DETAILS -------------same functions added into payout blade file-------------------------------------------------------
 ...
 var paymentValidator=$('#paymentForm').validate({ 
		...
		$(document).on('change','#payment_status',function()
        {
			...
			if(sopt.data('business')==0)
			{
				alert("Please set lead commission.!");
				$(this).val(cstatus);
			}
			else
			{
...
$(document).on('click','#view_receipt',function(e)
{
	var lead_id=$(this).data('lead-id');
		...
		$.ajax({
			url: "{{ route('admin.get-lead-payment-details') }}",
			type: 'get',
			dataType:'json',
			...
			success: function(res)
			{
				if(res.status==1)
				{
					$("#payCommission").html(res.data[0].commission);
					$("#paymentDate").html(res.data[0].payment_date);
					$("#paymentId").html(res.data[0].payment_id);
					$("#payReceipt").attr('src',res.data[0].payment_receipt);
				}
			}
		});
