		...
		$data=$dat->get();
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                           ...
                           $btn = '<button type="button" class="btn btn-outline btn-circle edit-partner" data-bs-toggle="modal" data-bs-target="#edit-partner-modal" id="'.$row->id.'" title="Edit"><i class="fa fa-pencil"></i></button>
...
public function createPartner(Request $request)
    {
	    ...
	    if ($validate->fails())
        {
			return response()->json(['status'=>0,'msg'=>'Email already exists. Try again!']);
        }
		
			try
			{
				$email_exist= Partner::where('email',$request->email)->first();
				$mob_exist= Partner::where('mobile',$request->mobile)->first();
				
				if(!$email_exist and !$mob_exist)
				{
					...
					if($result)
					{
						...
						//------SEND PARTNER TO CRM-----
						$send_response=$this->sendPartnerDetailToCrm($request); //send partner to getlead CRM
						//------------------------------
				...
				{
					return redirect()->back()->with('fail','A partner has already registered with this Email/Mobile number.');
				}
			}
			catch(\Exception $e)
			{
    ...
    {
         Partner::where('id',$request->partner_id)->first()->delete();
    ...
    public function createLead(Request $request)
    {
		...
		Try
		{
			...
			//"A new lead has been added to your account! Let's turn this opportunity into success!";
			...
			//"A new lead has been added for [Partner Name]. Check your email for details."
		...
		{
			\Log::info($e->getMessage());
			return response()->json(['status'=>0,'msg'=>'Something wrong, Try again.']);
		}
    ...
    public function listLeads(Request $request)
    {
            ...
            return Datatables::of($data)
					...
					->addColumn('status', function($row) use($lstatus)
					{
						...
						if($row->lead_status=="Got Business"){ $clas=' payment-active';}else {$clas='';}
					...
					->addColumn('pay_status', function($row)
					{
						...
						$payStatus="";
						$gotB="1";
						if($row->lead_status!="Got Business"){$gotB="0";}
												
						if($row->payment_status == 0){$clas=' payment-inactive';}else if($row->payment_status ==1){ $clas=' payment-active';} else {$clas=' payment-pending';}
						...
						$pay_status.= '<option  value="1" data-business="'.$gotB.'" '.($row->payment_status == 1 ? ' selected' : ''). ' >Paid</option>';
						$pay_status.= '<option  value="2"' . ($row->payment_status == 2 ? ' selected' : '') . '>Pending</option>';
						$pay_status.= '</select>';
						
						if($row->payment_status == 1)
						{
							$pay_status ="<a href='#' class='btn btn-outline' id='view_receipt' data-lead-id='".$row->id."' data-bs-toggle='modal' data-bs-target='#view-receipt-modal'><span style='font-size:14px;font-weight:600;color:green;'>Paid</span></a>";
						}
   ...
   /* public function updatePaymentStatus(Request $request)
    {
		...
		if($request->status==1)
		{
			if($lead->amount_collected!="" and $lead->commission_amount!="" and $lead->lead_status=="Got Business")
			{
				$lead->payment_status = $request->status;
				$lead->save();
				return response()->json(['status'=>1,'msg'=>'Payment completed!']);
			}
			else
			{
				return response()->json(['status'=>0,'msg'=>'Please accept lead commission. Thank You!']);	
			}
		}
		else
		{
...
public function gotBusinessUnPaidLeads(Request $request)
    {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('partner', function($row){
                        ...
                        return $row->partner_name."[".$row->id."]";
                    })
					->addColumn('amount_collected', function($row){
